...
class Sequence(keras.utils.Sequence):
    def __init__(self, ml_dataset: MLDataset, batch_size: int):
    ...
    def __getitem__(self, i):
        ...
        end_idx = min(i * self.batch_size + self.batch_size, self.n_examples)
        try:
            (
                inputs_batch,
                targets_batch,
                sample_weight_batch,
            ) = self.ml_dataset.get_batch_from_range(begin_idx, end_idx)
...
def from_ml_dataset(
    ...
    return Sequence(batch_size=batch_size, ml_dataset=ml_dataset)
