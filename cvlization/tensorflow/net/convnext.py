...
def forward_features(input, downsample_layers, stages):
    for i in range(4):
        y = downsample_layers[i](input)
        y = stages[i](y)
    return y
...
def create_model(
    ...
    **kwargs,
    ...
    x = tf.keras.Input(shape=(*input_shape, 3))
    y = net(x)
    ...
    if pretrained is True:
        ...
        if use_tpu:
            ...
            net.load_weights(pretrained_ckpt, options=load_locally, skip_mismatch=True)
        else:
            net.load_weights(pretrained_ckpt, skip_mismatch=True, by_name=True)

    model = tf.keras.Model(inputs=x, outputs=y, name=model_name)
    return model
