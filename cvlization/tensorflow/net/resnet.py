# flake8: noqa
"""
Adapted from: https://github.com/keras-team/keras-contrib/blob/master/keras_contrib/applications/resnet.py
...
def _conv_bn_relu_dropout(**conv_params):
    ...
    padding = conv_params.setdefault("padding", "same")
    kernel_regularizer = conv_params.setdefault("kernel_regularizer", l2(1.0e-4))
    spatial_dropout = SpatialDropout2D(0.2)
    ...
    def f(x):
        ...
        x = _bn_relu(x, bn_name=bn_name, relu_name=relu_name)
        x = spatial_dropout(x)
        return x
...
def _bn_relu_conv_dropout(**conv_params):
    """Helper to build a BN -> relu -> conv residual unit with full pre-activation
    function. This is the ResNet v2 scheme proposed in
    http://arxiv.org/pdf/1603.05027v2.pdf
    """
...
def ResNet(
    ...
    elif residual_unit == "v2d":
        residual_unit = _bn_relu_conv_dropout
    elif residual_unit == "v1d":
        residual_unit = _conv_bn_relu_dropout
...
def ResNet18(input_shape, **kwargs):
    ...
    return ResNet(input_shape, basic_block, repetitions=[2, 2, 2, 2], **kwargs)


def ResNet34(input_shape, **kwargs):
    """ResNet with 34 layers and v2 residual units"""
    return ResNet(input_shape, basic_block, repetitions=[3, 4, 6, 3], **kwargs)


def ResNet50(input_shape, **kwargs):
    """ResNet with 50 layers and v2 residual units"""
    return ResNet(input_shape, bottleneck, repetitions=[3, 4, 6, 3], **kwargs)


def ResNet101(input_shape, **kwargs):
    """ResNet with 101 layers and v2 residual units"""
    return ResNet(input_shape, bottleneck, repetitions=[3, 4, 23, 3], **kwargs)


def ResNet152(input_shape, **kwargs):
    """ResNet with 152 layers and v2 residual units"""
    return ResNet(input_shape, bottleneck, repetitions=[3, 8, 36, 3], **kwargs)


if __name__ == "__main__":
    _ = ResNet18(
        input_shape=(1200, 800, 1),
        include_top=False,
        final_pooling=None,
        residual_unit="v2d",
    )
