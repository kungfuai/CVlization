...
This script demonstrates how to train a conditional normalizing flow using the flow matching loss. The flow is trained on a simple 2D dataset and samples from the flow are generated.
...
if __name__ == '__main__':
    ...
    parser.add_argument('--project', type=str, default='flow-matching')
    ...
    if args.track:
        ...
        x_arr = data.numpy()
        plt.hist2d(*(x_arr.T), bins=64)
        # plt.savefig('moons.pdf')
        if args.track:
            # log to wandb
            wandb.log({"data/moons": wandb.Image(plt)})

    for step in tqdm(range(16384), ncols=88):
        ...
        if step % args.sample_every == 0:
            ...
            plt.hist2d(*(x_arr.T), bins=64)
            # plt.savefig('moons_cfm.pdf')
            if args.track:
                wandb.log({"sampled/moons_cfm": wandb.Image(plt)})
            ...
            # print(log_p)
            if args.track:
                wandb.log({"train/log_p": log_p.mean().item()})
    ...
    # if args.track:
    #     # log to wandb
    #     wandb.log({"sampled/moons_fm": wandb.Image(plt)})
