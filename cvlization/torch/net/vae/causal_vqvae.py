"""
Adapted from https://github.com/PKU-YuanGroup/Open-Sora-Plan/blob/main/opensora/models/ae/videobase/causal_vqvae/modeling_causalvqvae.py
"""

from .video_base import VideoBaseAE
import torch
...
import os
import json
from typing import Tuple, Dict, Union, TypeVar, Any
import yaml
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        for key, value in vars(self).items():
            if isinstance(value, VideoBaseConfiguration):
                d[key] = (
                    value.to_dict()
                )  # Serialize nested VideoBaseConfiguration instances
...
class SpatialDownsample2x(torch.nn.Module):
    def __init__(
        ...
        self.conv = torch.nn.Conv2d(
            self.chan_in, self.chan_out, self.kernel_size, stride=stride
        )
...
class TimeUpsample2x(nn.Module):
    ...
    def forward(self, x):
        ...
        if x.size(-1) > 1:
            ...
            #     (
            #         x[:, :, :1],
            #         F.interpolate(x[:, :, 1:], scale_factor=2.0, mode="linear"),
            #     ),
            #     dim=-1,
            # )
...
class Codebook(nn.Module):
    ...
    def forward(self, z):
        ...
        avg_min_distance = torch.mean(torch.min(distances, dim=1).values)
...
class Decoder(nn.Module):
    ...
    def forward(self, x):
        ...
        # print(f"after time_res_stack: {h.shape}")
        for conv in self.time_conv:
            h = F.relu(conv(h))
        # print(f"after time_conv: {h.shape}")
        h = self.spatial_res_stack(h)
        # print(f"after spatial_res_stack: {h.shape}")
...
class AxialAttention(nn.Module):
    ...
    def forward(self, q, k, v, decode_step, decode_idx):
        ...
        if self.causal:
            mask = (
                torch.tril(torch.ones(q.shape[-2], q.shape[-2]))
                if self.causal
                else None
            )
            if decode_step is not None and mask is not None:
...
class StandaloneEncoder(nn.Module):
    def __init__(self, encoder: nn.Module, n_hiddens: int, embedding_dim: int):
        super().__init__()
        self.encoder = encoder
        self.pre_vq_conv = CausalConv3d(n_hiddens, embedding_dim, 1)
...
class StandaloneDecoder(nn.Module):
    def __init__(self, decoder: nn.Module, n_hiddens: int, embedding_dim: int):
        super().__init__()
        self.decoder = decoder
        self.post_vq_conv = CausalConv3d(embedding_dim, n_hiddens, 1)

    def forward(self, x):
        x = self.post_vq_conv(x)
        # print("post_vq_conv:", x.shape)
...
class CausalVQVAEModel(VideoBaseAE):
    ...
    def encode(
        self, x: Tensor, include_embeddings: bool = False
