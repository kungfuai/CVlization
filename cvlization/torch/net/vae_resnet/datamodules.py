...
class ImageDataset(Dataset):
    ...
    def __init__(self, folder: str, image_size: int, ffcv: bool = False):
        ...
        self.samples = sorted(
            list(pathlib.Path(folder).rglob("*.png"))
            + list(pathlib.Path(folder).rglob("*.jpg"))
            + list(pathlib.Path(folder).rglob("*.bmp"))
            + list(pathlib.Path(folder).rglob("*.JPEG"))
        )
        self.ffcv = ffcv

        self.transforms = Compose(
            [ToTensor(), Resize((image_size, image_size), antialias=True)]
        )
        ...
        if stage == "fit":
            if self.train_folder is not None:
                self.train = ImageDataset(
                    self.train_folder, self.image_size, ffcv=False
                )
            ...
            if self.predict_folder is not None:
                self.predict = ImageDataset(
                    self.predict_folder, self.image_size, ffcv=False
                )
        else:
            pass
