...
def table_formatter(val, is_title=False, col_width=12, formats=None):
    ...
    return (
        formats["title"] if is_title else formats.get(type_(val), formats["default"])
    ...
    def append(self, data):
        ...
        if self.report(data):
            try:
                ...
                print(*(self.formatter(data[k]) for k in data.keys()))
            except KeyError:
                print("data keys now:" + str(data.keys()))
                raise
...
class ColorMap(dict):
    ...
    def _repr_html_(self):
        ...
        return (
            ...
            + css
            + "</style>"
            + "".join((s.format(color, text) for text, color in self.items()))
        )
...
def make_dot_graph(nodes, edges, direction="LR", **kwargs):
    ...
    class Subgraphs(dict):
        def __missing__(self, path):
            ...
            subgraph = Cluster(
                path, label=label, style="rounded, filled", fillcolor="#77777744"
            )
    ...
    for path, attr in nodes:
        parent, label = split(path)
        subgraphs[parent].add_node(Node(name=path, label=label, **attr))
...
class DotGraph:
    def __init__(self, graph, size=15, direction="LR"):
        ...
        self.edges = [
            (src, dst, {}) for dst, (_, inputs) in graph.items() for src in inputs
        ]
        self.size, self.direction = size, direction

    def dot_graph(self, **kwargs):
        return make_dot_graph(
