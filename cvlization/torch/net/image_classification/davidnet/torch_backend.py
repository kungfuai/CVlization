...
# GPU dataloading
chunks = lambda data, splits: (
    data[start:end] for (start, end) in zip(splits, splits[1:])
)
...
def transformed(data, targets, transform, max_options=None, unshuffle=False):
    ...
    data = torch.cat(
        [
            transform(x, **choice)
            for choice, x in zip(
                options, chunks(data, even_splits(len(data), len(options)))
            )
        ]
    )
...
def LARS_update(w, dw, v, lr, weight_decay, momentum):
    nesterov_update(
        w, dw, v, lr * (norm(w) / (norm(dw) + 1e-2)).to(w.dtype), weight_decay, momentum
    )
...
def forward_tta(tta_transforms):
    def step(batch, state):
        ...
        logits = torch.mean(
            torch.stack(
                [
                    model({"input": transform(batch["input"].clone())})[
                        "logits"
                    ].detach()
...
def update_ema(momentum, update_freq=1):
    ...
    def step(batch, state):
        if not batch:
            return
        if (next(n) % update_freq) != 0:
            return
        for v, ema_v in zip(
            state[MODEL].state_dict().values(), state[VALID_MODEL].state_dict().values()
        ):
            if not v.dtype.is_floating_point:
                continue  # skip things like num_batches_tracked.
            ema_v *= rho
            ema_v += (1 - rho) * v
...
def train_epoch(
    ...
    valid_summary, valid_time = epoch_stats(
        reduce(valid_batches, state, valid_steps)
    ), timer(
        include_in_total=False
    )  # DAWNBench rules
    print("train summary:", train_summary)
    print("validation summary:", valid_summary)
    return {
