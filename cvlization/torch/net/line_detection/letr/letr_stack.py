"""
Adapted from https://github.com/mlpc-ucsd/LETR

LETR consists of a coarse model and a fine model. Here we only include the coarse model
because it is simpler while not sacrificing accuracy significantly.
"""
...
from .util import nested_tensor_from_tensor_list
...
class LETRstack(nn.Module):
    def __init__(self, letr, args):
        ...
        self.transformer = Transformer(
            d_model=args.second_hidden_dim,
            dropout=args.second_dropout,
            nhead=args.second_nheads,
            dim_feedforward=args.second_dim_feedforward,
            num_encoder_layers=args.second_enc_layers,
            num_decoder_layers=args.second_dec_layers,
            normalize_before=args.second_pre_norm,
    ...
    def forward(self, samples, postprocessors=None, targets=None, criterion=None):
        ...
        # layer 1 transformer
        hs1, _ = self.letr.transformer(
            self.letr.input_proj(src1), mask1, self.letr.query_embed.weight, pos[l1_num]
        )
...
class Transformer(nn.Module):
    ...
    def forward(self, src, mask, query_embed, pos_embed):
        ...
        hs, attn_output_weights = self.decoder(
            tgt,
            memory,
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward_post(
        ...
        tgt = self.norm1(tgt)
        tgt2, attn_output_weights = self.multihead_attn(
