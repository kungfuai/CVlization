...
class LETR(nn.Module):
    ...
    def __init__(
        self, backbone, transformer, num_classes, num_queries, args, aux_loss=False
    ):
        super().__init__()
    ...
    def forward(self, samples, postprocessors=None, targets=None, criterion=None):
        ...
        hs = self.transformer(
            self.input_proj(src), mask, self.query_embed.weight, pos[num]
        )[0]
...
class SetCriterion(nn.Module):
    def __init__(self, num_classes, weight_dict, eos_coef, losses, args, matcher=None):
        ...
        self.args = args
        try:
            self.args.label_loss_params = eval(
                self.args.label_loss_params
            )  # Convert the string to dict.
        except:
            pass

    def loss_lines_labels(
        self, outputs, targets, num_items, log=False, origin_indices=None
    ):
        ...
        target_classes_o = torch.cat(
            [t["labels"][J] for t, (_, J) in zip(targets, origin_indices)]
        )
    ...
    def forward(self, outputs, targets, origin_indices=None):
        ...
        num_items = torch.as_tensor(
            [num_items], dtype=torch.float, device=next(iter(outputs.values())).device
        )
        ...
        for loss in self.losses:
            losses.update(
                self.get_loss(
                    loss, outputs, targets, num_items, origin_indices=origin_indices
                )
            )
