...
class Donut:
    ...
    def train(self, dataset_builder):
        ...
        train_dataloader, val_dataloader, newly_added_num = self._create_dataloaders(
            dataset_builder, processor
        )
        # newly_added_num: num of newly added tokens
        model = self._create_model(
            self.pretrained_model_name, newly_added_num, config, processor
        )
    ...
    def _create_processor(self):
        ...
        processor.feature_extractor.size = self._image_size[
            ::-1
    ...
    def _create_model(
        self, pretrained_model_name: str, newly_added_num: int, config, processor
    ):
    ...
    def _create_dataloaders(self, dataset_builder, processor):
        train_dataset = self._process_dataset(
            dataset_builder.training_dataset(), processor
        )
        val_dataset = self._process_dataset(
            dataset_builder.validation_dataset(), processor
        )
