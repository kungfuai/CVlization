...
def _get_checkpoint(model_type, checkpoint_path=None):
    if checkpoint_path is None:
        ...
        raise ValueError(
            f"The checkpoint path {checkpoint_path} that was passed does not exist."
        )
...
def get_custom_sam_model(
    ...
    state = torch.load(
        checkpoint_path, map_location=device, pickle_module=custom_pickle
    )
...
def _precompute_tiled_2d(predictor, input_, tile_shape, halo, f, verbose=True):
    ...
    for tile_id in tqdm(
        range(n_tiles),
        total=n_tiles,
        desc="Predict image embeddings for tiles",
        disable=not verbose,
    ):
...
def _precompute_tiled_3d(predictor, input_, tile_shape, halo, f, verbose=True):
    ...
    pbar = tqdm(
        total=n_tiles * n_slices,
        desc="Predict image embeddings for tiles and slices",
        disable=not verbose,
    )
...
def _precompute_2d(input_, predictor, save_path, tile_shape, halo):
    ...
    if "input_size" in f.attrs:  # the embeddings have already been precomputed
        ...
        use_tiled_prediction
    ):  # the embeddings have not been computed yet and we use tiled prediction
...
def _precompute_3d(
    input_, predictor, save_path, lazy_loading, tile_shape=None, halo=None
):
    f = zarr.open(save_path, "a")
    ...
    if "input_size" in f.attrs:  # the embeddings have already been precomputed
        ...
        for z, z_slice in tqdm(
            enumerate(input_), total=input_.shape[0], desc="Precompute Image Embeddings"
        ):
...
def precompute_image_embeddings(
    ...
    if tile_shape is not None:
        ...
        ), "Tiled prediction is only supported when the embeddings are saved to file."
    ...
    if save_path is not None:
        ...
        if (
            "input_size" in f.attrs
        ):  # we have computed the embeddings already and perform checks
    ...
    if ndim == 2:
        image_embeddings = (
            _compute_2d(input_, predictor)
            if save_path is None
            else _precompute_2d(input_, predictor, save_path, tile_shape, halo)
        )

    elif ndim == 3:
        image_embeddings = (
            _compute_3d(input_, predictor)
            if save_path is None
            else _precompute_3d(
                input_, predictor, save_path, lazy_loading, tile_shape, halo
            )
        )
...
def set_precomputed(
    predictor: SamPredictor, image_embeddings: ImageEmbeddings, i: Optional[int] = None
):
...
def load_image_data(
    ...
    if key is None:
        ...
        raise NotImplementedError("Only loading from image files is supported.")
