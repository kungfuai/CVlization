"""
Adapted from the trainer provided in:
...
class TorchEmTrainer:
    ...
    class Deserializer:
        ...
        def __init__(
            self, init_data: dict, save_path: str, device: Union[str, torch.device]
        ):
        ...
        def load_optimizer(self, kwarg_name: str, optional: bool):
            self.load_generic(
                kwarg_name, self.trainer_kwargs["model"].parameters(), optional=optional
            )
    ...
    def fit(
        ...
        for _ in range(train_epochs):
            ...
            if (
                save_every_kth_epoch is not None
                and (self._epoch + 1) % save_every_kth_epoch == 0
            ):
    ...
    def _train_epoch_impl(
        self, progress, forward_context, backprop: Callable[[torch.Tensor], None]
    ):
        self.model.train()
