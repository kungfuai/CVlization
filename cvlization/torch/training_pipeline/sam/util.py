...
def get_trainer(checkpoint, name="best", device=None):
    ...
    if isinstance(checkpoint, str):
        ...
        trainer = torch_em.trainer.DefaultTrainer.from_checkpoint(
            checkpoint, name=name, device=device
        )
    else:
        trainer = checkpoint
...
def get_normalizer(trainer):
    ...
    while isinstance(dataset, torch_em.data.concat_dataset.ConcatDataset) or isinstance(
...
def load_model(
    checkpoint, model=None, name="best", state_key="model_state", device=None
):
