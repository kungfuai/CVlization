...
class ConvertToSamInputs:
    ...
    def _get_prompt_lists(self, gt, n_samples, prompt_generator):
        ...
        if n_samples is None:  # n-samples is set to None, so we use all ids
            ...
            sampled_cell_ids = np.random.choice(
                cell_ids, size=min(n_samples, len(cell_ids)), replace=False
            )
            sampled_cell_ids = np.sort(sampled_cell_ids)

        # only keep the bounding boxes for sampled cell ids
        bbox_coordinates = [
            bbox_coordinates[sampled_id] for sampled_id in sampled_cell_ids
        ]

        # convert the gt to the one-hot-encoded masks for the sampled cell ids
        object_masks = segmentation_to_one_hot(
            gt, None if n_samples is None else sampled_cell_ids
        )

        # derive and return the prompts
        point_prompts, point_label_prompts, box_prompts, _ = prompt_generator(
    ...
    def __call__(self, x, y, n_pos, n_neg, get_boxes=False, n_samples=None):
        ...
        for image, gt in zip(x, y):
            ...
            if get_boxes:
                assert len(sampled_cell_ids) == len(
                    box_prompts
                ), f"{len(sampled_cell_ids)}, {len(box_prompts)}"

            if get_points:
                assert (
                    len(sampled_cell_ids)
                    == len(point_prompts)
                    == len(point_label_prompts)
