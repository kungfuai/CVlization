"""
ResNet based autoencoder for image compression.
"""
...
class ResBlock(nn.Module):
    ...
    def __init__(self, in_channels: int, out_channels: int = None):
        ...
        self.conv1 = nn.Conv2d(
            in_channels, out_channels, kernel_size=(3, 3), padding="same", bias=False
        )

        self.norm2 = nn.GroupNorm(num_groups=32, num_channels=out_channels)
        self.conv2 = nn.Conv2d(
            out_channels, out_channels, kernel_size=(3, 3), padding="same", bias=False
        )
...
class Encoder(nn.Module):
    def __init__(
        ...
        self.conv_in = torch.nn.Conv2d(
            3, channels, kernel_size=(3, 3), stride=(1, 1), padding=1, bias=False
        )
        ...
        self.final_residual = nn.Sequential(
            *[ResBlock(ch_in) for _ in range(num_res_blocks)]
        )

        self.norm = nn.GroupNorm(num_groups=32, num_channels=ch_in)
        self.conv_out = torch.nn.Conv2d(
            ch_in, embedding_dim, kernel_size=(1, 1), padding="same"
        )
...
class Decoder(nn.Module):
    def __init__(
        ...
        self.conv_in = torch.nn.Conv2d(
            embedding_dim, ch_in, kernel_size=(3, 3), stride=(1, 1), padding=1
        )
        ...
        self.conv_out = torch.nn.Conv2d(
            channels, 3, kernel_size=(3, 3), stride=(1, 1), padding=1
        )
