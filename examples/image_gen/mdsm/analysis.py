...
def save_sample_pdf(samples, grid_size, filename, pad=2):
    ...
    last_step_denoise = (
        make_grid(samples, normalize=False, nrow=grid_size[1], padding=pad)
        .detach()
        .cpu()
        .numpy()
    )
...
def get_inception_logits(inception, data_in, gpu):
    ...
    for i in range(N_batch):
        data = data_in[i * max_batch : (i + 1) * max_batch]
        data = I_std * F.layer_norm(data, (32, 32)) + I_mean
        expanded_data = F.interpolate(
            data, size=(299, 299), mode="bilinear", align_corners=False
        )
        # expanded_data = 2*expanded_data - 1
        act.append(
            inception(expanded_data.clamp(min=-1, max=1))
            .detach()
            .cpu()
            .squeeze()
            .numpy()
...
def FID(mean1, mean2, cov1, cov2):
    ...
    out = ((mean1 - mean2) ** 2).sum() + np.trace(
        cov1 + cov2 - 2 * linalg.sqrtm(cov1 @ cov2)
    )
