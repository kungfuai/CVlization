            ...
            for _ in range(num_clips_per_video):
                ...
                with torch.no_grad():
                    ...
                    if isinstance(vae, AutoencoderKL):
                        ...
                        latent_multiplier = 9
                        latent_bias = -0.27
                        x = z * latent_multiplier + latent_bias  # TODO: this is specific to the VAE! Should be automatically estimated from the VAE.
                        
                        x_unscaled = rearrange(x_unscaled, '(b f) d h w -> b f d h w', b=b).contiguous()
                ...
                if args.sample_every > 0 and train_steps % args.sample_every == 0 and train_steps > 0:
                    ...
                    if isinstance(vae, AutoencoderKL):
                        ...
                        decoded_samples = vae.decoder((samples - latent_bias) / latent_multiplier)
