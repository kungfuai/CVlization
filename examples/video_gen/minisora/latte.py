...
class TransformerBlock(nn.Module):
    ...
    def forward(self, x, c):
        ...
        # x: torch.Size([64, 256, 384]), c: torch.Size([8, 384]), shift_msa: torch.Size([8, 384]), scale_msa: torch.Size([8, 384]), gate_msa: torch.Size([8, 384]), shift_mlp: torch.Size([8, 384]), scale_mlp: torch.Size([8, 384]), gate_mlp: torch.Size([8, 384])
        # print(f"x: {x.shape}, c: {c.shape}, shift_msa: {shift_msa.shape}, scale_msa: {scale_msa.shape}, gate_msa: {gate_msa.shape}, shift_mlp: {shift_mlp.shape}, scale_mlp: {scale_mlp.shape}, gate_mlp: {gate_mlp.shape}")
...
class Latte(nn.Module):
    ...
    def forward(self, 
        ...
        assert height == self.input_size, f"Input height {height} does not match model input size {self.input_size}"
        ...
        # print(f"self.pos_embed: {self.pos_embed.shape}, self.temp_embed: {self.temp_embed.shape}")
        # print(f"x: {x.shape}, t: {t.shape}, timestep_spatial: {timestep_spatial.shape}, timestep_temp: {timestep_temp.shape}")
