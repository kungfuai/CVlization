...
class CFGDenoiserWithGrad(CompVisDenoiser):
    ...
    def cond_model_fn_(self, x, sigma, inner_model=None, **kwargs):
        ...
        for cond_fn in self.cond_fns:
            ...
            # Gradient wrt x0_pred, so save some compute: don't record grad until after denoised is calculated
                ...
                with torch.enable_grad():
                    cond_grad = cond_fn(
                        x, sigma, denoised=denoised.detach().requires_grad_(), **kwargs
                    ).detach()
            total_cond_grad += cond_grad

        total_cond_grad = torch.nan_to_num(
            total_cond_grad, nan=0.0, posinf=float("inf"), neginf=-float("inf")
        )
        ...
        if self.gradient_wrt == "x":
            x.copy_(
                self.grad_consolidate_fn(
                    x.detach(), total_cond_grad, k_utils.append_dims(sigma, x.ndim)
                )
            )
        ...
        if self.check_conditioning_schedule(sigma):
            ...
            # Calculate cond and uncond separately
                ...
                if self.gradient_add_to == "uncond":
                    ...
                    raise Exception(
                        f"Unrecognised option for gradient_add_to: {self.gradient_add_to}"
                    )
    ...
    def make_cond_fn(self, loss_fn, scale):
        ...
        if self.gradient_wrt == "x":
            ...
            raise Exception(
                f"Variable gradient_wrt == {self.gradient_wrt} not recognised."
            )
    ...
    def check_conditioning_schedule(self, sigma):
        ...
        if self.cond_fns is not None and any(
            cond_fn is not None for cond_fn in self.cond_fns
        ):
