...
class DepthModel:
    ...
    def load_adabins(self, models_path):
        ...
        self.adabins_helper = InferenceHelper(
            models_path, dataset="nyu", device=self.device
        )
    ...
    def predict(self, prev_img_cv2, anim_args) -> torch.Tensor:
        ...
        if use_adabins:
            ...
            img_pil = Image.fromarray(
                cv2.cvtColor(prev_img_cv2.astype(np.uint8), cv2.COLOR_RGB2BGR)
            )
            image_pil_area = w * h
            resized = True
        ...
        if self.midas_model is not None:
            ...
            sample = (
                torch.from_numpy(img_midas_input).float().to(self.device).unsqueeze(0)
            )
            ...
            if use_adabins:
                depth_map = midas_depth * anim_args.midas_weight + adabins_depth * (
                    1.0 - anim_args.midas_weight
                )
            else:
