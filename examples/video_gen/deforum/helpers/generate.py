...
def generate(
    args, root, frame=0, return_latent=False, return_sample=False, return_c=False
):
    seed_everything(args.seed)
    os.makedirs(args.outdir, exist_ok=True)

    sampler = (
        PLMSSampler(root.model) if args.sampler == "plms" else DDIMSampler(root.model)
    )
    ...
    if args.init_latent is not None:
        ...
        with precision_scope("cuda"):
            init_latent = root.model.get_first_stage_encoding(
                root.model.encode_first_stage(args.init_sample)
            )
    ...
    assert not (
        (args.use_mask and args.overlay_mask)
        and (args.init_sample is None and init_image is None)
    ...
    if (
        args.init_mse_scale
        and args.init_mse_image != None
        and args.init_mse_image != ""
    ):
    ...
    assert not (
        ...
        and (args.init_mse_image is None or args.init_mse_image == "")
    ...
    args.clamp_schedule = dict(
        zip(
            k_sigmas.tolist(),
            np.linspace(args.clamp_start, args.clamp_stop, args.steps + 1),
        )
    )
    ...
    if args.colormatch_scale != 0:
        assert (
            args.colormatch_image is not None
        ), "If using color match loss, colormatch_image is needed"
        ...
        if args.decode_method == "linear":
            mse_loss_fn = make_mse_loss(
                root.model.linear_decode(
                    root.model.get_first_stage_encoding(
                        root.model.encode_first_stage(init_mse_image.to(root.device))
                    )
                )
            )
        else:
    ...
    assert not (
        any([cond_fs[1] != 0 for cond_fs in loss_fns_scales])
        and (args.sampler in ["ddim", "plms"])
    ...
    grad_inject_timing_fn = make_inject_timing_fn(
        args.grad_inject_timing, model_wrap, args.steps
    )
            ...
            with root.model.ema_scope():
                for cond_prompts, uncond_prompts in zip(cond_data, uncond_data):
                    ...
                    if args.sampler in [
                        ...
                        if init_latent is not None and args.strength > 0:
                            z_enc = sampler.stochastic_encode(
                                init_latent,
                                torch.tensor([t_enc] * batch_size).to(root.device),
                            )
                    ...
                    if args.use_mask and args.overlay_mask:
                        ...
                        if args.mask_sample is None or args.using_vid_init:
                            args.mask_sample = prepare_overlay_mask(
                                args, root, img_original.shape
                            )

                        x_samples = img_original * args.mask_sample + x_samples * (
                            (args.mask_sample * -1.0) + 1
                        )
                    ...
                    for x_sample in x_samples:
                        ...
                        x_sample = 255.0**exponent_for_rearrange * rearrange(
