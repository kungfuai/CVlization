...
def get_ffmpeg_path(ffmpeg_outdir, ffmpeg_timestring, ffmpeg_extension):
    try:
        ffmpeg_image_path = os.path.join(
            ffmpeg_outdir, ffmpeg_timestring + f"_%05d.{ffmpeg_extension}"
        )
...
def make_mp4_ffmpeg(ffmpeg_args, display_ffmpeg=False, debug=False):
    ...
    cmd = [
        ...
        "png",
        "-r",
        str(ffmpeg_args.ffmpeg_fps),
        "-start_number",
        str(0),
        "-i",
        ffmpeg_args.ffmpeg_image_path,
        "-frames:v",
        str(ffmpeg_args.ffmpeg_maxframes),
        "-c:v",
        "libx264",
        "-vf",
