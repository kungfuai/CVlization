...
def download_model(model_map, root):
    ...
    # write to model path
    with open(
        os.path.join(root.models_path, root.model_checkpoint), "wb"
    ) as model_file:
        file_size = int(ckpt_request.headers.get("Content-Length"))
        with tqdm(
            total=file_size, unit="B", unit_scale=True, desc=root.model_checkpoint
        ) as pbar:
...
def load_model(root, load_on_run_all=True, check_sha256=True, map_location="cuda"):
    ...
    # config path
    ckpt_config_path = (
        root.custom_config_path
        if root.model_config == "custom"
        else os.path.join(root.configs_path, root.model_config)
    )
    ...
    ckpt_path = (
        ...
        else os.path.join(root.models_path, root.model_checkpoint)
    )
    ckpt_valid = True
    ...
    if os.path.exists(ckpt_path):
        ...
        print(
            f"Please download model checkpoint and place in {os.path.join(root.models_path, root.model_checkpoint)}"
        )
        ckpt_valid = False
