...
def vid2frames(video_path, frames_path, n=1, overwrite=True):
    if not os.path.exists(frames_path) or overwrite:
        ...
        while success:
            if count % n == 0:
                cv2.imwrite(
                    frames_path + os.path.sep + f"{t:05}.jpg", image
...
def anim_frame_warp(
    prev, args, anim_args, keys, frame_idx, depth_model=None, depth=None, device="cuda"
):
    ...
    if anim_args.animation_mode == "2D":
        ...
        prev_img = anim_frame_warp_3d(
            device, prev_img_cv2, depth, anim_args, keys, frame_idx
        )
...
def anim_frame_warp_2d(prev_img_cv2, args, anim_args, keys, frame_idx):
    ...
    if anim_args.flip_2d_perspective:
        ...
        post_trans_mat = np.float32(
            [[1, 0, (args.W - sl) / 2], [0, 1, (args.H - sl) / 2]]
        )
...
class DeformAnimKeys:
    def __init__(self, anim_args):
        self.angle_series = get_inbetweens(
            parse_key_frames(anim_args.angle), anim_args.max_frames
        )
        self.zoom_series = get_inbetweens(
            parse_key_frames(anim_args.zoom), anim_args.max_frames
        )
        self.translation_x_series = get_inbetweens(
            parse_key_frames(anim_args.translation_x), anim_args.max_frames
        )
        self.translation_y_series = get_inbetweens(
            parse_key_frames(anim_args.translation_y), anim_args.max_frames
        )
        self.translation_z_series = get_inbetweens(
            parse_key_frames(anim_args.translation_z), anim_args.max_frames
        )
        self.rotation_3d_x_series = get_inbetweens(
            parse_key_frames(anim_args.rotation_3d_x), anim_args.max_frames
        )
        self.rotation_3d_y_series = get_inbetweens(
            parse_key_frames(anim_args.rotation_3d_y), anim_args.max_frames
        )
        self.rotation_3d_z_series = get_inbetweens(
            parse_key_frames(anim_args.rotation_3d_z), anim_args.max_frames
        )
        ...
        self.noise_schedule_series = get_inbetweens(
            parse_key_frames(anim_args.noise_schedule), anim_args.max_frames
        )
        ...
        self.sampler_schedule_series = get_inbetweens(
            parse_key_frames(anim_args.sampler_schedule),
            anim_args.max_frames,
            is_single_string=True,
        )
        self.kernel_schedule_series = get_inbetweens(
            parse_key_frames(anim_args.kernel_schedule), anim_args.max_frames
        )
        self.sigma_schedule_series = get_inbetweens(
            parse_key_frames(anim_args.sigma_schedule), anim_args.max_frames
        )
        self.amount_schedule_series = get_inbetweens(
            parse_key_frames(anim_args.amount_schedule), anim_args.max_frames
        )
        self.threshold_schedule_series = get_inbetweens(
            parse_key_frames(anim_args.threshold_schedule), anim_args.max_frames
        )
        self.hybrid_comp_alpha_schedule_series = get_inbetweens(
            parse_key_frames(anim_args.hybrid_comp_alpha_schedule), anim_args.max_frames
        )
        self.hybrid_comp_mask_blend_alpha_schedule_series = get_inbetweens(
            parse_key_frames(anim_args.hybrid_comp_mask_blend_alpha_schedule),
            anim_args.max_frames,
        )
        self.hybrid_comp_mask_contrast_schedule_series = get_inbetweens(
            parse_key_frames(anim_args.hybrid_comp_mask_contrast_schedule),
            anim_args.max_frames,
        )
        self.hybrid_comp_mask_auto_contrast_cutoff_high_schedule_series = (
            get_inbetweens(
                parse_key_frames(
                    anim_args.hybrid_comp_mask_auto_contrast_cutoff_high_schedule
                ),
                anim_args.max_frames,
            )
        )
        self.hybrid_comp_mask_auto_contrast_cutoff_low_schedule_series = get_inbetweens(
            parse_key_frames(
                anim_args.hybrid_comp_mask_auto_contrast_cutoff_low_schedule
            ),
            anim_args.max_frames,
        )
        ...
        if not value_is_number:
            ...
            if is_single_string:
                ...
                if value.find('"') > -1:
                    value = value.replace('"', "")
            key_frame_series[i] = (
                numexpr.evaluate(value) if not is_single_string else value
            )  # workaround for values formatted like 0:("I am test") //used for sampler schedules
    key_frame_series = (
        key_frame_series.astype(float) if not is_single_string else key_frame_series
    )  # as string
