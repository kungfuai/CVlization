...
try:
    ...
    running = subprocess.run(
        ["pip", "install", "numpngw"], stdout=subprocess.PIPE
    ).stdout.decode("utf-8")
    print(running)
    from numpngw import write_png
...
def convert_image_to_8bpc(image, bit_depth_output):
    if bit_depth_output == 16:
        ...
        image = np.clip(
            image * 256, 0, 255
        )  # Clip values below 0 and above 255 (but those values ARE PRESENT in the EXRs)
        image = Image.fromarray(image.astype("uint8"))
    return image
...
def save_8_16_or_32bpc_image(image, outdir, filename, bit_depth_output):
    if bit_depth_output == 8:
        ...
        cv2.imwrite(
            os.path.join(outdir, filename).replace(".png", ".exr"),
...
def render_image_batch(root, args, cond_prompts, uncond_prompts):
    ...
    if len(cond_prompts) > len(uncond_prompts):
        uncond_prompts += [uncond_prompts[-1]] * (
            len(cond_prompts) - len(uncond_prompts)
        )
    ...
    for iprompt, (cond_prompt, uncond_prompt) in enumerate(
        zip(cond_prompts, uncond_prompts)
    ):
            ...
            for image in init_array:  # iterates the init images
                ...
                for image in results:
                    ...
                    if args.save_samples:
                        ...
                        save_8_16_or_32bpc_image(
                            image, args.outdir, filename, args.bit_depth_output
                        )
...
def render_animation(root, anim_args, args, cond_prompts, uncond_prompts):
    ...
    if anim_args.animation_mode in ["2D", "3D"]:
        if anim_args.hybrid_composite or anim_args.hybrid_motion in [
            "Affine",
            "Perspective",
            "Optical Flow",
        ]:
    ...
    predict_depths = (
        anim_args.animation_mode == "3D" and anim_args.use_depth_warping
    ) or anim_args.save_depth_maps
    predict_depths = predict_depths or (
        anim_args.hybrid_composite
        and anim_args.hybrid_comp_mask_type in ["Depth", "Video Depth"]
    )
    if predict_depths:
    ...
    if anim_args.resume_from_timestring and not using_vid_init:
        ...
        if turbo_steps > 1:
            turbo_next_image, turbo_next_frame_idx = (
                sample_to_cv2(prev_sample, type=np.float32),
                last_frame,
            )
    ...
    while frame_idx < anim_args.max_frames:
        ...
        hybrid_comp_schedules = {
            ...
            "mask_blend_alpha": keys.hybrid_comp_mask_blend_alpha_schedule_series[
                frame_idx
            ],
            "mask_contrast": keys.hybrid_comp_mask_contrast_schedule_series[frame_idx],
            "mask_auto_contrast_cutoff_low": int(
                keys.hybrid_comp_mask_auto_contrast_cutoff_low_schedule_series[
                    frame_idx
                ]
            ),
            "mask_auto_contrast_cutoff_high": int(
                keys.hybrid_comp_mask_auto_contrast_cutoff_high_schedule_series[
                    frame_idx
                ]
            ),
        }
        ...
        if turbo_steps > 1:
            ...
            for tween_frame_idx in range(tween_frame_start_idx, frame_idx):
                ...
                if advance_prev:
                    turbo_prev_image, _ = anim_frame_warp(
                        turbo_prev_image,
                        args,
                        anim_args,
                        keys,
                ...
                if advance_next:
                    turbo_next_image, _ = anim_frame_warp(
                        turbo_next_image,
                        args,
                        anim_args,
                        keys,
                ...
                if tween_frame_idx > 0:
                    if anim_args.hybrid_motion in ["Affine", "Perspective"]:
                        if anim_args.hybrid_motion_use_prev_img:
                            if advance_prev:
                                matrix = get_matrix_for_hybrid_motion_prev(
                                    tween_frame_idx,
                                    (args.W, args.H),
                                    inputfiles,
                                    turbo_prev_image,
                                ...
                                turbo_prev_image = image_transform_ransac(
                                    turbo_prev_image,
                                    matrix,
                                    anim_args.hybrid_motion,
                                    cv2.BORDER_WRAP
                            ...
                            if advance_next:
                                ...
                                turbo_next_image = image_transform_ransac(
                                    turbo_next_image,
                                    matrix,
                                    anim_args.hybrid_motion,
                                    cv2.BORDER_WRAP
                            ...
                            matrix = get_matrix_for_hybrid_motion(
                                tween_frame_idx - 1,
                                (args.W, args.H),
                                inputfiles,
                                anim_args.hybrid_motion,
                    ...
                    if anim_args.hybrid_motion in ["Optical Flow"]:
                        if anim_args.hybrid_motion_use_prev_img:
                            if advance_prev:
                                ...
                                turbo_prev_image = image_transform_optical_flow(
                                    turbo_prev_image,
                                    flow,
                                    cv2.BORDER_WRAP
                                    if anim_args.border == "wrap"
                            ...
                            flow = get_flow_for_hybrid_motion(
                                tween_frame_idx - 1,
                                (args.W, args.H),
                                inputfiles,
                                hybrid_frame_path,
                ...
                if args.use_mask and args.overlay_mask:
                    ...
                    init_image_raw, _ = anim_frame_warp(
                        args.init_sample_raw,
                        args,
                        anim_args,
                        keys,
                        frame_idx,
                    ...
                    args.init_sample_raw = (
                        sample_from_cv2(init_image_raw).half().to(root.device)
                    )

                # Transform the mask image
                if args.use_mask:
                    if args.mask_sample is None:
                        args.mask_sample = prepare_overlay_mask(
                            args, root, prev_sample.shape
                        )
                    # Transform the mask
                    mask_image, _ = anim_frame_warp(
                        args.mask_sample,
                        args,
                        anim_args,
                        keys,
                    ...
                    args.mask_sample = (
                        sample_from_cv2(mask_image).half().to(root.device)
                    )
                ...
                if anim_args.save_depth_maps:
                    depth_model.save(
                        os.path.join(
                            args.outdir,
                            f"{args.timestring}_depth_{tween_frame_idx:05}.png",
                        ),
                        depth,
                        args.bit_depth_output,
                    )
        ...
        if prev_sample is not None:
            prev_img, depth = anim_frame_warp(
                prev_sample,
                args,
                anim_args,
                keys,
            ...
            if frame_idx > 0:
                if anim_args.hybrid_motion in ["Affine", "Perspective"]:
                    ...
                    prev_img = image_transform_ransac(
                        prev_img,
                        matrix,
                        anim_args.hybrid_motion,
                        cv2.BORDER_WRAP
                ...
                if anim_args.hybrid_motion in ["Optical Flow"]:
                    ...
                    prev_img = image_transform_optical_flow(
                        prev_img,
                        flow,
                        cv2.BORDER_WRAP
                        if anim_args.border == "wrap"
                ...
                if anim_args.hybrid_use_video_as_mse_image:
                    args.init_mse_image = os.path.join(
                        args.outdir, "inputframes", f"{frame_idx:05}.jpg"
                    )
            ...
            if anim_args.hybrid_composite:
                args, prev_img = hybrid_composite(
                    args,
                    anim_args,
                    frame_idx,
                    prev_img,
            ...
            if anim_args.color_coherence != "None":
                ...
                hybrid_available = (
                    anim_args.hybrid_composite
                    or anim_args.hybrid_motion
                    in ["Optical Flow", "Affine", "Perspective"]
                )
                if anim_args.color_coherence == "Video Input" and hybrid_available:
                    video_color_coherence_frame = (
                        int(frame_idx)
                        % int(anim_args.color_coherence_video_every_N_frames)
                        == 0
                    )
                    if video_color_coherence_frame:
                        prev_vid_img = Image.open(
                            os.path.join(
                                args.outdir, "inputframes", f"{frame_idx:05}.jpg"
                            )
                        )
                        prev_vid_img = prev_vid_img.resize(
                            (args.W, args.H), Image.Resampling.LANCZOS
                        )
            ...
            # apply anti-blur
            contrast_sample = unsharp_mask(
                contrast_sample, (kernel, kernel), sigma, amount, threshold
            )
        ...
        if not using_vid_init:
            ...
            print(
                f"Angle: {keys.angle_series[frame_idx]} Zoom: {keys.zoom_series[frame_idx]}"
            )
            print(
        ...
        if using_vid_init:
            init_frame = os.path.join(
                args.outdir, "inputframes", f"{frame_idx+1:05}.jpg"
            )
            print(f"Using video init frame {init_frame}")
            args.init_image = init_frame
            if anim_args.use_mask_video:
                mask_frame = os.path.join(
                    args.outdir, "maskframes", f"{frame_idx+1:05}.jpg"
                )
                args.mask_file = mask_frame

        # sample the diffusion model
        sample, image = generate(
            args, root, frame_idx, return_latent=False, return_sample=True
        )
        ...
        if turbo_steps > 1:
            ...
            turbo_next_image, turbo_next_frame_idx = (
                sample_to_cv2(sample, type=np.float32),
                frame_idx,
            )
            ...
            if anim_args.save_depth_maps:
                ...
                depth_model.save(
                    os.path.join(
                        args.outdir, f"{args.timestring}_depth_{frame_idx:05}.png"
                    ),
                    depth,
                    args.bit_depth_output,
                )
...
def render_input_video(root, anim_args, args, cond_prompts, uncond_prompts):
    ...
    vid2frames(
        anim_args.video_init_path,
        video_in_frame_path,
        anim_args.extract_nth_frame,
    ...
    if anim_args.use_mask_video:
        ...
        print(
            f"Exporting Video Frames (1 every {anim_args.extract_nth_frame}) frames to {mask_in_frame_path}..."
        )
        vid2frames(
            anim_args.video_mask_path,
            mask_in_frame_path,
            anim_args.extract_nth_frame,
            anim_args.overwrite_extracted_frames,
        )
...
def render_interpolation(root, anim_args, args, cond_prompts, uncond_prompts):
    ...
    if anim_args.interpolate_key_frames:
        for i in range(len(prompts_c_s) - 1):
            dist_frames = (
                list(cond_prompts.items())[i + 1][0] - list(cond_prompts.items())[i][0]
            )
            if dist_frames <= 0:
