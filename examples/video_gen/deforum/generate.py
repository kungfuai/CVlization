...
def DeforumAnimArgs():
    ...
    sampling_mode = (
        "bicubic"  # @param ['bicubic', 'bilinear', 'nearest'] {type:'string'}
    )
...
def DeforumArgs():
    ...
    clip_name = (
        "ViT-L/14"  # @param ['ViT-L/14', 'ViT-L/14@336px', 'ViT-B/16', 'ViT-B/32']
    )
...
if __name__ == "__main__":
    ...
    def ffmpegArgs():
        ...
        if ffmpeg_mode == "auto":
            ffmpeg_outdir, ffmpeg_timestring = get_auto_outdir_timestring(
                args, ffmpeg_mode
            )
