...
def generalized_box_iou(boxes1, boxes2):
    ...
    #             boxes1[i] = torch.zeros_like(boxes1[i])
    assert (boxes1[:, 2:] >= boxes1[:, :2]).all(), "error boxes: {} vs {}.".format(boxes1, boxes2)
    assert (boxes2[:, 2:] >= boxes2[:, :2]).all(), "error boxes: {} vs {}.".format(boxes1, boxes2)
