...
This script demonstrates multimodal understanding using Qwen3-VL-8B-Instruct from Alibaba,
a 8B parameter model with enhanced visual perception and reasoning capabilities.
...
def run_inference(model, processor, images, prompt: str):
    """
    Run inference on one or more images using Qwen3-VL-8B-Instruct.
...
Examples:
  ...
  python predict.py --image doc.jpg --task ocr --output result.json --format json
    ...
    print("=" * 60)
    print(f"Qwen3-VL-8B-Instruct - Alibaba Cloud")
