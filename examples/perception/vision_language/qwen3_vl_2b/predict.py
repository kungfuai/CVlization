#!/usr/bin/env python3
"""
Qwen3-VL-2B-Instruct - Alibaba Cloud's Advanced Vision Language Model

This script demonstrates multimodal understanding using Qwen3-VL-2B-Instruct from Alibaba,
a 2B parameter model with enhanced visual perception and reasoning capabilities.
...
def run_inference(model, processor, images, prompt: str):
    """
    Run inference on one or more images using Qwen3-VL-2B-Instruct.
    ...
    with torch.no_grad():
        generated_ids = model.generate(
            **inputs,
            max_new_tokens=128  # Reduced from 512 for concise answers
        )
...
Examples:
  ...
  python predict.py --image doc.jpg --task ocr --output result.json --format json
    ...
    print("=" * 60)
    print(f"Qwen3-VL-2B-Instruct - Alibaba Cloud")
