...
IMG="${CVL_IMAGE:-whisperx}"

CACHE_ROOT="${HOME}/.cache/cvlization"
MODEL_CACHE="${CACHE_ROOT}/whisperx/models"
HF_CACHE="${HF_HOME:-${CACHE_ROOT}/huggingface}"
TORCH_CACHE="${TORCH_HOME:-${CACHE_ROOT}/torch}"

mkdir -p "${CACHE_ROOT}" "${MODEL_CACHE}" "${HF_CACHE}" "${TORCH_CACHE}"
...
docker "${DOCKER_ARGS[@]}" \
  ...
  --mount "type=bind,src=${MODEL_CACHE},dst=/workspace/models" \
  ...
  --env "MODEL_BASE=/workspace/models" \
