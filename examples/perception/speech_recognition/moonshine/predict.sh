...
# Moonshine targets resource-constrained hardware -- default to CPU so the
# example matches its on-device positioning. Override with USE_GPU=1 or
# GPU_DEVICE=N (GPU isn't required, but useful-moonshine's torch backend
# will use it if visible).
