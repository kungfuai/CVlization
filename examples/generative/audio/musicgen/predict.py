#!/usr/bin/env python3
"""
MusicGen text-to-music generation with AudioCraft.
...
DEFAULT_PROMPT = "upbeat electronic music with a warm bassline and light percussion"
HF_DATA_REPO = "zzsi/cvl"
HF_DATA_PREFIX = "musicgen"
SAMPLE_FILES = {
    "bach.mp3": "sample_data/bach.mp3",
}
...
def detect_device(requested_device: Optional[str] = None) -> str:
    ...
    if hasattr(torch.backends, "mps") and torch.backends.mps.is_available():
        ...
        return "mps"
    print("Using CPU. MusicGen inference will be slow.")
    return "cpu"
...
def generate_music(
    ...
    if melody_path:
        print(f"Loading melody conditioning audio: {melody_path}")
        melody_audio, sample_rate = sf.read(melody_path, dtype="float32", always_2d=True)
        melody = torch.from_numpy(melody_audio.T)
        print("Generating music with melody conditioning...")
        return model.generate_with_chroma([prompt], melody[None], sample_rate, progress=progress)

    print("Generating music from text...")
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Generate music from text with AudioCraft MusicGen.",
    ...
    parser.add_argument("--output", type=str, default="musicgen.wav", help="Output WAV path")
    parser.add_argument("--model", type=str, default=DEFAULT_MODEL, help="MusicGen model id")
    parser.add_argument("--duration", type=float, default=10.0, help="Generated audio duration in seconds")
    parser.add_argument(
        "--melody",
        type=str,
        default=None,
        help="Optional melody audio path for melody-capable models. Use 'sample' for the bundled Bach sample.",
    )
...
def main() -> int:
    ...
    if args.melody == "sample":
        melody_path = str(ensure_sample_data() / "bach.mp3")
    else:
        melody_path = resolve_input_path(args.melody, input_dir) if args.melody else None
    ...
    wav = generate_music(model, prompt, melody_path, progress=not args.no_progress)
