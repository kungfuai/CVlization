...
elif [ "${CVL_USE_GPU:-${USE_GPU:-1}}" = "1" ]; then
  DOCKER_ARGS+=(--gpus=all)
fi
if [ -n "${HF_TOKEN:-}" ]; then
...
docker "${DOCKER_ARGS[@]}" \
  ...
  --env "QWEN3_OMNI_MODEL_ID=${QWEN3_OMNI_MODEL_ID:-}" \
  --env "QWEN3_OMNI_QUANT=${QWEN3_OMNI_QUANT:-}" \
  --env "QWEN3_OMNI_SPEAKER=${QWEN3_OMNI_SPEAKER:-}" \
  --env "QWEN3_OMNI_SYSTEM=${QWEN3_OMNI_SYSTEM:-}" \
  --env "QWEN3_OMNI_MAX_NEW_TOKENS=${QWEN3_OMNI_MAX_NEW_TOKENS:-}" \
  --env "QWEN3_OMNI_TEMPERATURE=${QWEN3_OMNI_TEMPERATURE:-}" \
  --env "QWEN3_OMNI_TEXT_ONLY=${QWEN3_OMNI_TEXT_ONLY:-}" \
