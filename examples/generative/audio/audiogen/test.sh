...
OUTPUT="${SCRIPT_DIR}/outputs/test_audiogen.wav"

mkdir -p "${HOME}/.cache/huggingface"
mkdir -p "${HOME}/.cache/torch"
mkdir -p "${SCRIPT_DIR}/outputs"

echo "=== AudioGen Smoke Test ==="
...
docker run --rm --gpus=all \
  ...
  "$IMG" python predict.py \
    --model debug \
    --text "a dog barking twice in a quiet room" \
    --duration 1 \
    --prompt-audio sample \
    --prompt-duration 0.25 \
    --seed 123 \
    --output test_audiogen.wav \
    --no-progress

echo "=== Verifying output ==="
if [ -f "$OUTPUT" ]; then
  SIZE=$(stat -f%z "$OUTPUT" 2>/dev/null || stat -c%s "$OUTPUT" 2>/dev/null)
  echo "Output file: outputs/test_audiogen.wav"
