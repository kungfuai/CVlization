...
# Stage 1 training: RAE autoencoder decoder training with GAN + LPIPS losses
# Requires ImageNet dataset in ImageFolder format
# This trains the decoder - encoder (DINOv2) is frozen
...
if [ -z "${CONFIG:-}" ]; then
    case "$DATASET" in
        cifar10|cifar100|animalfaces)
            CONFIG="configs/training/stage1_DINOv2-B_decXL_cifar.yaml"
            ;;
        *)
            CONFIG="configs/training/stage1_DINOv2-B_decXL.yaml"
            ;;
    esac
fi
DATA_PATH="${DATA_PATH:-}"
