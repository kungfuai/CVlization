...
class WanModel(torch.nn.Module):
    def __init__(
        ...
        if controls is not None:
            if len(controls) > 0:
                old_conv = self.patch_embedding
                in_channel = old_conv.in_channels + 16 * (len(controls)-1) + 1
                new_conv = nn.Conv3d(
                    in_channel, old_conv.out_channels, old_conv.kernel_size, old_conv.stride, old_conv.padding
                ).to(dtype=torch.bfloat16)
                new_conv.weight.data[:, :old_conv.in_channels] = old_conv.weight.data
                new_conv.weight.data[:, old_conv.in_channels:] = 0
...
class WanModelStateDictConverter:
    ...
    def from_civitai(self, state_dict):
        ...
        elif hash_state_dict_keys(state_dict) == "54e443997354404240816e6b6526b822":
            # WorldCanvas
            config = {
                "has_image_input": False,
                "patch_size": [1, 2, 2],
