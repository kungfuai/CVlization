...
class ModelLogger:
    ...
    # def save_model(self, accelerator, model, step):
    #     accelerator.wait_for_everyone()
    #     if accelerator.is_main_process:
    #         # 1. Save resumable state
    #         accelerator.save_state(os.path.join(self.resumable_path, f"step-{step}"))
    #         print(f"Saved resumable checkpoint for step {step}")

    #         # --- 步骤 2: 保存用于推理的、包含新结构和权重的完整模型 ---
            
    #         # 首先，获取最原始的模型对象
    #         unwrapped_model = accelerator.unwrap_model(model)
            
    #         # 定义保存目录，每个step一个子目录
    #         save_directory = os.path.join(self.portable_path, f"step-{step}")
            
    #         # 使用 save_pretrained 保存模型和配置文件
    #         # 这会自动保存 config.json 和权重文件
    ...
    #             output_folder = os.path.join(self.validation_path, f"step-{step}")
    #             os.makedirs(output_folder, exist_ok=True)
    #             for i, config in enumerate(self.validation_config):
    #                 with torch.no_grad():
    #                     video = pipe(**config)
    #                 imageio.mimsave(os.path.join(output_folder, f"sample-{i}.mp4"), video, fps=30)
    #             print(f"Validation samples saved to {output_folder}")
    
    # def on_step_end(self, loss, accelerator, model, global_step):
    #     accelerator.wait_for_everyone()
    #     if accelerator.is_main_process:
    ...
    #         path = os.path.join(self.output_path, f"step-{global_step}.safetensors")
    ...
    def save_model(self, accelerator, model, file_name):
        ...
        # 2. 第二个同步点！
        #    确保所有进程都等待，直到主进程创建完目录。
        #    当代码执行过这一行时，可以保证目录一定存在了。
        accelerator.wait_for_everyone()

        # 3. 所有进程一起调用 save_state
        accelerator.save_state(path)
