...
class FusedAdam(torch.optim.Optimizer):
    ...
    def step(self, closure=None, grads=None, output_params=None, scale=None, grad_norms=None, grad_scaler=None):
        ...
        for group, group_master in zip(self.param_groups, self.param_groups_master):
            ...
            if self.capturable:
                ...
                found_inf = grad_scaler._check_inf_per_device(self)[device] if grad_scaler is not None else torch.zeros((1,), device=device)
    ...
    def load_state_dict(self, state_dict):
        ...
        for group in self.param_groups:
            if self.capturable:
                group["lr"] = group["lr"].cuda() if isinstance(group["lr"], torch.Tensor) else torch.tensor(group["lr"], dtype=torch.float32).cuda()
