...
The upstream DiT inference pipeline is designed for structured JSON prompts
generated by the Rewriter model. Raw natural-language prompts (--prompt) are
best-effort; for official-quality results, use --prompt-json with a structured
caption (see upstream docs/en/dit_inference.md).
...
# Canonical structured prompt hosted on HuggingFace (upstream t2v example_1)
CANONICAL_PROMPT_URL = (
    "https://huggingface.co/datasets/zzsi/cvl/resolve/main/"
    "lingbot_video/canonical_t2v_prompt.json"
)
...
def caption_from_sample(sample: dict) -> str:
    ...
    Follows the upstream lingbot_video.utils.caption_from_sample logic:
    extracts the 'caption' key, then serialises dicts/lists to compact JSON.
    """
    if "caption" in sample:
        caption = sample["caption"]
    else:
        runtime_keys = {"duration", "fps", "height", "width", "num_frames", "resolution", "ratio"}
        caption = {k: v for k, v in sample.items() if k not in runtime_keys}
    if isinstance(caption, (dict, list)):
        return json.dumps(caption, ensure_ascii=False, separators=(",", ":"))
    return str(caption)


def load_prompt_json(path: str) -> tuple:
    """Load a structured prompt JSON file and return (caption_str, duration).

    The structured prompt format is documented in upstream
    docs/en/dit_inference.md. The caption is serialised to a compact JSON
    string as expected by the text encoder.
    """
    with open(path, "r", encoding="utf-8") as f:
        data = json.load(f)
    caption_str = caption_from_sample(data)
    duration = data.get("duration", None)
    return caption_str, duration
...
def generate_video(
    ...
    cfg_mode = "batched" if batch_cfg else "sequential"
    print(
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Raw text prompt (best-effort; model was trained on structured "
             "JSON captions from the Rewriter — use --prompt-json for official "
             "quality). Ignored when --prompt-json is provided.",
    )
    parser.add_argument(
        "--prompt-json",
        type=str,
        default=None,
        help="Path to a structured prompt JSON file (upstream Rewriter format). "
             "This is the official input format and produces the best results. "
             "See docs/en/dit_inference.md in the upstream repo for the schema.",
    )
    parser.add_argument(
        "--negative-prompt",
        type=str,
        default="",
        help="Negative prompt for quality guidance (auto-loaded from package if empty)",
    )
    parser.add_argument(
        "--image",
        type=str,
        default=None,
        help="Conditioning image for text+image-to-video (TI2V) mode",
    )
    parser.add_argument(
        "--output",
        type=str,
        default="output.mp4",
        help="Output file path (use .png for T2I mode)",
    )
    ...
    if args.prompt_json:
        ...
        if not os.path.exists(prompt_json_path):
            print(f"Error: Prompt JSON not found: {prompt_json_path}", flush=True)
            sys.exit(1)
        prompt, duration = load_prompt_json(prompt_json_path)
        print(f"Loaded structured prompt from: {prompt_json_path}", flush=True)
        if duration:
            print(f"  duration: {duration}s", flush=True)
    elif args.prompt:
        prompt = args.prompt
        print("Using raw text prompt (best-effort; use --prompt-json for "
              "official quality)", flush=True)
    else:
        print("Error: Provide --prompt or --prompt-json", flush=True)
        sys.exit(1)
