        ...
        for motion_module in motion_modules:
            ...
            if model_config.path != "":
                if model_config.path.endswith(".ckpt"):
                    ...
                    state_dict = {}
                    with safe_open(
                        model_config.path, framework="pt", device="cpu"
                    ) as f:
                        for key in f.keys():
                    ...
                    if not is_lora:
                        ...
                        base_state_dict = {}
                        with safe_open(
                            model_config.base, framework="pt", device="cpu"
                        ) as f:
                            for key in f.keys():
                    ...
                    if is_lora:
                        pipeline = convert_lora(
                            pipeline, state_dict, alpha=model_config.lora_alpha
                        )

                    # additional networks
                    if (
                        hasattr(model_config, "additional_networks")
                        and len(model_config.additional_networks) > 0
                    ):
                        for lora_weights in model_config.additional_networks:
                            ...
                            with safe_open(
                                lora_path.strip(), framework="pt", device="cpu"
                            ) as f:
                                for key in f.keys():
                                    add_state_dict[key] = f.get_tensor(key)
                            pipeline = convert_lora(
                                pipeline, add_state_dict, alpha=lora_alpha
                            )
            ...
            init_image = (
                model_config.init_image if hasattr(model_config, "init_image") else None
            )
