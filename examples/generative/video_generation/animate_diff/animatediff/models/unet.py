...
class UNet3DConditionModel(ModelMixin, ConfigMixin):
    ...
    @classmethod
    def from_pretrained_2d_zz(
        cls, pretrained_model_path, subfolder=None, unet_additional_kwargs=None
    ):
        model_name = "models--" + pretrained_model_path.replace("/", "--")
        if subfolder is not None:
            pretrained_model_path = os.path.join(pretrained_model_path, subfolder)
        print(
            f"loaded temporal unet's pretrained weights from {pretrained_model_path} ..."
        )
        snapshot_dirs = os.listdir(
            os.path.join(huggingface_cache_dir(), model_name, "snapshots")
        )
        snapshot_dirs[0]
        ...
        pipeline = StableDiffusionPipeline.from_single_file(model_file)
