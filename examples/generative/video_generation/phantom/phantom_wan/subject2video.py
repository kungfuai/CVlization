...
class Phantom_Wan_S2V:
    ...
    def __init__(
        ...
        logging.info(f"Creating WanModel from {phantom_ckpt}")
        self.model = WanModel(dim=config.dim, 
    ...
    def generate(self,
        ...
        ref_latents = [self.get_vae_latents(ref_images, self.device)]
        ref_latents_neg = [torch.zeros_like(ref_latents[0])]

        F = frame_num
        target_shape = (self.vae.model.z_dim, (F - 1) // self.vae_stride[0] + 1 + ref_latents[0].shape[1],
        ...
        with amp.autocast(dtype=self.param_dtype), torch.no_grad(), no_sync():
            ...
            x0 = latents
            x0 = [x0_[:,:-ref_latents[0].shape[1]] for x0_ in x0]
