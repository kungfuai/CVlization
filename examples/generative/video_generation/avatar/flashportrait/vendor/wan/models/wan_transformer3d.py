...
class WanI2VCrossAttention(WanSelfAttention):
    ...
    def __init__(self,
        ...
        self.emo_k_proj = nn.Linear(2048, dim, bias=False)
        self.emo_v_proj = nn.Linear(2048, dim, bias=False)

    def forward(self, x, context, context_lens, dtype=torch.bfloat16, t=0, emo_proj=None, emo_context_lens=None, latents_num_frames=21, ip_scale=1.0, emo_attn_mask=None):
        r"""
        Args:
            x(Tensor): Shape [B, L1, C]
            context(Tensor): Shape [B, L2, C]
        ...
        if len(emo_proj.shape) == 4:
            ...
            emo_x = attention(q.to(dtype), ip_key.to(dtype), ip_value.to(dtype), attn_mask=emo_attn_mask)
            emo_x = emo_x.flatten(2)
        ...
        mean_emo, std_emo = torch.mean(emo_x, dim=(1, 2), keepdim=True), torch.std(emo_x, dim=(1, 2), keepdim=True)
        emo_x = (emo_x - mean_emo) * (std_x / (std_emo + 1e-5)) + mean_x
...
class WanAttentionBlock(nn.Module):
    ...
    def forward(
        ...
        # print(f"dtype: {dtype}")
        # print(f"temp_x: {temp_x.dtype}")
        # print(f"e[2]: {e[2].dtype}")
        # print(f"y: {y.dtype}")
        # print(f"x-2: {x.dtype}")
        # print("-----------------------------------")

        # cross-attention & ffn function
        def cross_attn_ffn(x, context, context_lens, e, emo_proj=None, emo_context_lens=None, latents_num_frames=21, ip_scale=1.0, emo_attn_mask=None):
            # cross-attention
        ...
        x = cross_attn_ffn(x, context, context_lens, e, emo_proj=emo_proj, emo_context_lens=emo_context_lens, latents_num_frames=latents_num_frames, ip_scale=ip_scale, emo_attn_mask=emo_attn_mask)
        return x
...
class Head(nn.Module):
    ...
    def forward(self, x, e):
        ...
        # print("----------------------------------")
        # print(f"x: {x.dtype}")
        # print(f"e[1]: {e[1].dtype}")
        # print(f"e[0]: {e[0].dtype}")
        # print(f"head: {self.head.weight.dtype}")
        # print("----------------------------------")
...
class WanTransformer3DModel(ModelMixin, ConfigMixin, FromOriginalModelMixin):
    ...
    def __init__(
        ...
        # self.portrait_encoder = PortraitEncoder(adapter_in_dim=768, adapter_proj_dim=2048)
    ...
    def init_weights(self):
        ...
        for name, m in self.named_modules():
            if isinstance(m, nn.Linear):
                if ("emo_k_proj" in name) or ("emo_v_proj" in name):
                        ...
                        for key in missing_keys:
                            ...
                            if 'weight' in key:
                                if any(norm_type in key for norm_type in ['norm', 'ln_', 'layer_norm', 'group_norm', 'batch_norm']):
                                    ...
                                    initialized_dict[key] = torch.randn(param_shape, dtype=param_dtype) * 0.02
                                if 'emo_k_proj' in key or 'emo_v_proj' in key:
                                    initialized_dict[key] = torch.zeros(param_shape, dtype=param_dtype)
