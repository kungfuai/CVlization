...
class DDIMScheduler(SchedulerMixin, ConfigMixin):
    ...
    def set_timesteps(self, num_inference_steps: int, device: Union[str, torch.device] = None, num_train_timesteps = None):
        """
    ...
    def step(
        ...
        """
        if self.num_inference_steps is None and self.step_length is None:
            raise ValueError(
        ...
        if self.step_length is not None:
            prev_timestep = timestep - self.step_length
        else:
    ...
    def add_noise(
        ...
        shape_len = len(original_samples.shape)
        if shape_len == 5:
            b, c, f, h, w = original_samples.shape
            original_samples = rearrange(original_samples, "b c f h w -> (b f) c h w")
        ...
        if shape_len == 5:
            noisy_samples = rearrange(noisy_samples, "(b f) c h w -> b c f h w", b=b)
