...
| **E2.5** | Flux vs SD-1.5 portrait A/B (same 4 prompts: realistic / anime / dwarf / kitchen) on drifter (RTX PRO 6000 Blackwell) | ✅ done | Flux dominates across all 4 categories — clearly stronger on stylized prompts (anime, dwarf) and photoreal humans. SD-1.5 is ~5× faster (~3s vs ~16s/image at 768x768) but quality and style-adherence gap is large. **Decision**: use Flux as the primary portrait generator for the KD dataset. SD skipped unless a low-fidelity subset is desired later for robustness. |
