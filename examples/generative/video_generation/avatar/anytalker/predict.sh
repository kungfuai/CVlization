...
IMG="${CVL_IMAGE:-anytalker}"

# Ensure cache directories exist
mkdir -p "${HOME}/.cache/huggingface"
mkdir -p "${HOME}/.cache/torch"
mkdir -p "${HOME}/.cache/cvlization/anytalker/checkpoints"
...
docker run --rm --gpus=all --shm-size 16G \
    ...
    --mount "type=bind,src=${HOME}/.cache/cvlization/anytalker/checkpoints,dst=/workspace/AnyTalker/checkpoints" \
