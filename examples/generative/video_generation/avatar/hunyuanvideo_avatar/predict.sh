...
IMG="${CVL_IMAGE:-hunyuanvideo_avatar}"
GPU_DEVICE="${GPU_DEVICE:-${CVL_GPU:-}}"

CACHE_ROOT="${HOME}/.cache/cvlization"
WEIGHTS_DIR="${CACHE_ROOT}/hunyuanvideo_avatar/weights"
...
docker run --rm "${GPU_FLAG[@]}" --shm-size 16G \
    ...
    --mount "type=bind,src=${WEIGHTS_DIR},dst=/workspace/HunyuanVideo-Avatar/weights" \
