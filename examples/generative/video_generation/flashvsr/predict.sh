...
IMG="${CVL_IMAGE:-cvlization/flashvsr:latest}"

CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-0}"
HF_CACHE="${HF_HOME:-${HOME}/.cache/huggingface}"
MODELS_DIR="${FLASHVSR_MODELS_DIR:-}"
...
if [[ -n "${MODELS_DIR}" ]]; then
    mkdir -p "${MODELS_DIR}"
    MODEL_MOUNT="--mount type=bind,src=${MODELS_DIR},dst=/models"
    MODEL_ARG="--weights_dir /models"
fi
...
docker run --rm --gpus=all \
    ...
    "$IMG" \
    python predict.py ${MODEL_ARG:+"${MODEL_ARG}"} "$@"
