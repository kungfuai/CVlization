...
parser.add_argument("--moe", action="store_true", help="enable Mixture of Experts (replaces standard MLP)")
parser.add_argument("--num-experts", type=int, default=8, help="number of expert MLPs (only used with --moe)")
parser.add_argument("--experts-per-tok", type=int, default=2, help="top-k experts per token (only used with --moe)")
parser.add_argument("--no-shared-expert", action="store_true", help="disable always-on shared expert (only used with --moe)")
...
if args.moe:
    print0(f"MoE enabled: {args.num_experts} experts, top-{args.experts_per_tok}, shared_expert={not args.no_shared_expert}")
...
model_config_kwargs = dict(
    ...
    # MoE config (only used if --moe flag is set)
    use_moe=args.moe,
    num_experts=args.num_experts,
    num_experts_per_tok=args.experts_per_tok,
    moe_shared_expert=not args.no_shared_expert,
)
...
while True:
    ...
    # Handle None gradients (can happen with MoE when some experts get no tokens)
