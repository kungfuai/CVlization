...
class SongUnet(torch.nn.Module):
    def __init__(
        ...
        self.embed_s = torch.nn.Sequential(
            Linear(noise_channels, emb_channels, **init),
            torch.nn.SiLU(),
        ...
        for level, mult in enumerate(channel_mult):
            ...
            if level == 0:
                ...
                if encoder_type == "skip":
                    self.enc[f"{res}x{res}_aux_down"] = Conv1d(
                        in_channels=caux,
                        out_channels=caux,
                        kernel=0,
                        down=True,
                    ...
                    self.enc[f"{res}x{res}_aux_skip"] = Conv1d(
                        in_channels=caux, out_channels=cout, kernel=1, **init
                    )
                if encoder_type == "residual":
                    self.enc[f"{res}x{res}_aux_residual"] = Conv1d(
                        in_channels=caux,
                        out_channels=cout,
                        kernel=3,
                        down=True,
        ...
        for level, mult in reversed(list(enumerate(channel_mult))):
            ...
            if decoder_type == "skip" or level == 0:
                if decoder_type == "skip" and level < len(channel_mult) - 1:
                    self.dec[f"{res}x{res}_aux_up"] = Conv1d(
                        in_channels=out_channels,
                        out_channels=out_channels,
                        kernel=0,
                        up=True,
                ...
                self.dec[f"{res}x{res}_aux_conv"] = Conv1d(
                    in_channels=cout, out_channels=out_channels, kernel=3, **init_zero
                )

    def forward(self, x, s, t, jvp_attention: bool = False, augment_labels=None):
        t = t.view(-1)
        s = s.view(-1)
        # reparam
        t = t - s
        # swap classes and sequence positions
        x = rearrange(x, "... k d -> ... d k")
        ...
        if self.map_augment is not None and augment_labels is not None:
            emb_t = emb_t + self.map_augment(augment_labels)
