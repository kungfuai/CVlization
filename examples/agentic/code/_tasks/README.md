...
5. Every agent's `evaluate.sh` adapter picks it up automatically — no
   per-agent wiring needed

## Adapter contract (per agent)

Each coding-agent preset has an `evaluate.sh` (registered as the
`evaluate` preset in its `example.yaml`, matching the
`autogen_pair_programmer` convention) that:
...
See `examples/agentic/code/pi/evaluate.sh` for the reference
implementation.
