...
python -m scripts.base_train --depth=4 --max_seq_len=512 --device_batch_size=1 --eval_tokens=512 --total_batch_size=512 --num_iterations=20
"""
...
from nanoproof.data.nemotron_dataloader import tokenizing_distributed_data_loader, tokenizing_distributed_data_loader_with_state
...
# Model architecture
depth = 26 # the depth of the Transformer model to train, rest of the kwargs are derived
max_seq_len = 768 # max context length
...
total_batch_size = 491520 # total desired batch size, in #tokens
...
wandb_run = DummyWandb() if use_dummy_wandb else wandb.init(project="nanoproof", name=run, config=user_config)
