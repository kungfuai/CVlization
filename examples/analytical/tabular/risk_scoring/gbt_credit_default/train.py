...
DATA_DIR = Path("data")
RAW_DATA_PATH = DATA_DIR / "german_credit.csv"
...
def download_dataset() -> None:
    ...
    print(f"Downloading credit risk dataset from {DATA_URL} ...")
...
def load_and_preprocess() -> Tuple[pd.DataFrame, pd.Series]:
    ...
    # Ensure binary labels as integers
    if df[TARGET_COLUMN].dtype == object:
        df[TARGET_COLUMN] = df[TARGET_COLUMN].str.lower().map({"good": 0, "bad": 1})
