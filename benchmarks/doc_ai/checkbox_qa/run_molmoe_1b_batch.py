#!/usr/bin/env python3
"""
Run MolmoE-1B batch_predict on CheckboxQA dev subset.

MolmoE-1B is a compact 1.2B (active) parameter VLM optimized for OCR and document understanding.
"""
...
MOONDREAM_DIR = REPO_ROOT / "examples/perception/vision_language/molmoe_1b"
...
def run_batch_predict(
    ...
    print(f"Running MolmoE-1B batch_predict in Docker...")
...
def main():
    parser = argparse.ArgumentParser(
        description="Run MolmoE-1B on CheckboxQA benchmark",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # MolmoE-1B with 1 page
  ./run_molmoe_1b_batch.py --max-pages 1 --track

  # With custom prompt and image size
  ./run_molmoe_1b_batch.py --max-pages 2 --max-image-size 1024 --prompt-template "{question}"

  # Different model revision
  ./run_molmoe_1b_batch.py --revision 2024-05-20
"""
    )
    ...
    parser.add_argument(
        "--model-id",
        type=str,
        default="vikhyatk/molmoe_1b",
        help="HuggingFace model ID (default: vikhyatk/molmoe_1b)"
    )
    parser.add_argument(
        "--revision",
        type=str,
        default="0924",
        help="Model revision (default: 0924)"
    )
    parser.add_argument(
        "--max-pages",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Maximum image dimension. Images larger than this will be resized. Default: no resizing"
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
    ...
    if args.output_dir is None:
        ...
        args.output_dir = SCRIPT_DIR / f"results/molmoe_1b_{args.max_pages}p{size_suffix}"

    # Setup paths
    subset_file = SCRIPT_DIR / "data/subset_dev.jsonl"
    batch_input_file = SCRIPT_DIR / "data/batch_input_molmoe_1b.jsonl"
    predictions_file = args.output_dir / "predictions.jsonl"
    results_file = args.output_dir / "eval_results.json"

    print(f"\n{'='*60}")
    print(f"MolmoE-1B CheckboxQA Benchmark")
    print(f"{'='*60}")
    ...
    if args.track:
        if not TRACKIO_AVAILABLE:
            ...
            size_suffix = f"_{args.max_image_size}px" if args.max_image_size else ""
            run_name = f"molmoe_1b_{args.max_pages}p{size_suffix}"
