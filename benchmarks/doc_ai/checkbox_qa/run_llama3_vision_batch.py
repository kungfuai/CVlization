#!/usr/bin/env python3
"""
Run Llama 3.2 Vision batch_predict on CheckboxQA dev subset.

Llama 3.2 Vision is Meta's 11B parameter VLM with 4-bit quantization via Unsloth.
Strong multimodal understanding with Llama 3 instruction-following capabilities.
"""
...
LLAMA3_VISION_DIR = REPO_ROOT / "examples/perception/vision_language/llama3_vision"
...
def run_batch_predict(
    ...
    cmd = [
        ...
        "-v", f"{LLAMA3_VISION_DIR}:/workspace",
    ...
    print(f"Running Llama 3.2 Vision batch_predict in Docker...")
...
def main():
    parser = argparse.ArgumentParser(
        description="Run Llama 3.2 Vision on CheckboxQA benchmark",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Llama 3.2 Vision with 1 page
  ./run_llama3_vision_batch.py --max-pages 1 --track

  # With custom prompt and token limit
  ./run_llama3_vision_batch.py --max-pages 2 --max-tokens 128 --prompt-template "{question}"

  # Full dev subset with 3 pages
  ./run_llama3_vision_batch.py --max-pages 3 --max-tokens 512
"""
    )
    ...
    parser.add_argument(
        "--model-id",
        type=str,
        default="unsloth/Llama-3.2-11B-Vision-Instruct-bnb-4bit",
        help="HuggingFace model ID (default: unsloth/Llama-3.2-11B-Vision-Instruct-bnb-4bit)"
    )
    parser.add_argument(
        "--max-pages",
        type=int,
    ...
    if args.output_dir is None:
        args.output_dir = SCRIPT_DIR / f"results/llama3_vision_{args.max_pages}p"

    # Setup paths
    subset_file = SCRIPT_DIR / "data/subset_dev.jsonl"
    batch_input_file = SCRIPT_DIR / "data/batch_input_llama3_vision.jsonl"
    predictions_file = args.output_dir / "predictions.jsonl"
    results_file = args.output_dir / "eval_results.json"

    print(f"\n{'='*60}")
    print(f"Llama 3.2 Vision CheckboxQA Benchmark")
    print(f"{'='*60}")
    print(f"Model: {args.model_id}")
    print(f"Max pages: {args.max_pages}")
    print(f"Max tokens: {args.max_tokens}")
    print(f"Max image size: {args.max_image_size or 'original'}")
    ...
    if args.track:
        if not TRACKIO_AVAILABLE:
            ...
            run_name = f"llama3_vision_{args.max_pages}p"
